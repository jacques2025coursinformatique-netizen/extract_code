...
class ApplicationUI:
    ...
    def _build_layout(self):
        ...
        # COLONNE GAUCHE
        # -------------------------
        Label(self.frame_left, text="Projet").pack(pady=5)
        Button(self.frame_left, text="Sélectionner un dossier", command=self.on_select_project).pack(pady=5)
        Button(self.frame_left, text="Ouvrir le dossier sélectionné", command=self.on_open_folder).pack(pady=5)

        Label(self.frame_left, text="Extraction complète").pack(pady=5)
        Button(self.frame_left, text="Extraire l'ensemble du projet", command=self.on_extract_full).pack(pady=5)

        Label(self.frame_left, text="Versions disponibles").pack(pady=5)

        # --- Frame versions + scrollbar ---
        frame_versions = Frame(self.frame_left)
        frame_versions.pack(fill="both", expand=True)

        self.list_versions = Listbox(frame_versions, selectmode=SINGLE)
        ...
        Button(self.frame_left, text="Restaurer la version sélectionnée", command=self.on_restore_full).pack(pady=5)
        Button(self.frame_left, text="Supprimer la version sélectionnée", command=self.on_delete_version).pack(pady=5)

        # -------------------------
        # COLONNE DROITE
        # -------------------------
        Label(self.frame_right, text="Fichiers de la version sélectionnée").pack(pady=5)

        # --- Frame files + scrollbar ---
        frame_files = Frame(self.frame_right)
        frame_files.pack(fill="both", expand=True)

        self.list_files = Listbox(frame_files, selectmode=MULTIPLE)
        ...
        Button(self.frame_right, text="Créer selected_context.md & .html", command=self.on_export_selected).pack(pady=5)
        Button(self.frame_right, text="Restaurer les fichiers sélectionnés", command=self.on_restore_selected).pack(pady=5)
